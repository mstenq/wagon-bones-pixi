...
function modifierCircleClass(polarity: GameModifierPolarity, compact: boolean): string {
  const colorClass = polarity === 'positive' ? 'bg-ui-modifier-positive' : 'bg-ui-modifier-negative';
        ...
        {hasModifiers ? (
          <div className="flex flex-wrap items-center justify-center gap-1 lg:gap-1.5">
            {modifiers.map((modifier) => (
              <span key={modifier.id} className={modifierCircleClass(modifier.polarity, compact)} aria-hidden />
            ))}
          </div>
        ) : (
